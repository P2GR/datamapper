...
- Applied SoftDeletes read scope consistently across validated lookups, related queries, counts, distinct counts, QueryBuilder scalar and aggregate APIs, and eager-loaded relations.
- Preserved explicit `with_softdeleted()`, `only_softdeleted()`, and `where('deleted_at', ...)` overrides without duplicate predicates.
- Kept hard-delete behavior as default when a model uses `SoftDeletes`; soft-delete writes require `protected $soft_delete_writes = true`.
- Kept `force_delete()` hard and restricted `restore()` to models with soft-delete writes enabled.
